...
bot.command('filter_p',async (ctx)=>{
  ...
  statisProxy = statisProxy.filter((one)=>{
    if((one.success<=0 && one.fails>=30) || ((one.fails / one.success)>=50 && (one.fails / one.success)<Infinity)){
      deadProxies.push(one)
    }
    return !((one.success<=0 && one.fails>=30) || ((one.fails / one.success)>=50 && (one.fails / one.success)<Infinity));
  })
